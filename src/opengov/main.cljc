(ns opengov.main
  "Kotodama WASM entrypoint for the Opengov clean-room actor (L4) — Clojure port.
  ...
  facts (`opengov.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "opengov")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"opengov.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "opengov-compat" :tier tier :entities entities} 200])
...
(defn start! [] :opengov-compat/ready)
